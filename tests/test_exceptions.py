...
def test_hash_mismatch_error_truncates_digests_in_message() -> None:
    ...
    message keeps only the first 16 characters. The full digests stay
    available via the .expected / .actual attributes."""
...
def test_transaction_error_operation_depends_on_rollback_flag() -> None:
    """The operation tag flips between commit and rollback so log filters
    can target one phase without parsing the message."""
